...
def bfs(queue) :
    while queue :
        row, col, depth = deque.popleft(queue)
        ...
        for di, dj in delta :
            ...
            if visited[nextRow][nextCol] <= depth + 1 : continue
            if data[nextRow][nextCol] == 2 : visited[nextRow][nextCol] = depth
            else : visited[nextRow][nextCol] = depth + 1
            deque.append(queue, (nextRow, nextCol, depth + 1))
...
for case in permu :
    ...
    queue = deque()
    for ele in case :
        deque.append(queue, (ele[0], ele[1], 0))
